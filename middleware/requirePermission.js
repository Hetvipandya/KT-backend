...
const requirePermission = (moduleName, requiredLevel) => {
  return async (req, res, next) => {
    try {
      ...
      if (!roleDoc) {
        // Attempt on-the-fly default roles seeding in case they were never seeded for this company
        try {
          const roleService = require('../services/role.service');
          await roleService.seedDefaultRoles(companyId.toString());
          roleDoc = await Role.findOne({
            companyId,
            name: new RegExp(`^${accessEntry.role}$`, 'i')
          });
        } catch (seedErr) {
          // Ignore seeding failure/conflicts and fallback to final check
        }
      }
