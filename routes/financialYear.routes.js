...
const { createFinancialYear, listFinancialYears, lockFinancialYear, deleteFinancialYear } = require('../controllers/financialYear.controller');
 ...
 * /api/financial-year:
 *   get:
 *     summary: List all financial years for a branch
 *     description: Retrieves financial years belonging to the supplied branch. The company is resolved from the branch.
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: query
...
/**
 * @openapi
 * /api/financial-year/{id}:
 *   delete:
 *     summary: Delete a financial year
 *     description: Deletes a financial year if no transactions exist.
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
 ...
 *       400:
 *         description: Financial year has associated transactions
 *       403:
 *         description: Access denied (Data isolation violation)
 */
router.delete('/:id', checkCompanyAccess, deleteFinancialYear);
