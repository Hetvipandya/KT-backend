...
/**
 * Get current balance of an account by aggregating all transaction history.
...
const getAccountBalance = async (accountId) => {
  ...
  const history = await ledgerService.getLedgerHistory(accountId.toString(), {
    companyId: account.companyId.toString()
  });

  return history.closingBalance;
};
...
const hasTransactions = async (accountId) => {
  ...
  return Array.isArray(history.entries) && history.entries.length > 0;
};
...
const seedDefaultCoa = async (companyId) => {
  ...
  const defaultCOA = [
    ...
    { name: 'Input GST', type: 'Asset', isGroup: false, code: '1410', parentName: 'Current Assets' },
    ...
    { name: 'Loans (Liability)', type: 'Liability', isGroup: true, code: '2210', parentName: 'Liabilities' },
    ...
    { name: 'Output GST', type: 'Liability', isGroup: false, code: '2200', parentName: 'Duties & Taxes' },
