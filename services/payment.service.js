...
const validateReferences = async (data) => {
  ...
  if (data.bankAccountId) {
    ...
    // Auto-resolve cash or active bank account
    const account = await BankAccount.findOne({
      companyId: data.companyId,
      ...(data.mode === 'CASH' ? { accountType: 'Cash' } : {}),
      isActive: true
    }) || await BankAccount.findOne({ companyId: data.companyId, isActive: true });

    if (account) {
      data.bankAccountId = account._id;
      return { financialYear, customer, bankAccount: account };
    }
  }
...
const applyAllocations = async (payment, allocations, invoices) => {
  for (const allocation of allocations) {
    ...
    invoice.amountReceived = (invoice.amountReceived || 0) + allocation.allocatedAmount;
    invoice.balanceDue = Math.max(0, invoice.grandTotal - invoice.amountReceived);
    invoice.status = invoice.balanceDue <= EPSILON ? 'PAID' : 'PARTIALLY_PAID';
    if (!invoice.appliedPayments.includes(payment._id)) {
