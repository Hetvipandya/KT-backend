 ...
 * Get current balance of a bank account's linked COA ledger.
 * Delegates to Module 3's coa.service.getAccountBalance().
 * Auto-syncs openingBalance from BankAccount to ChartOfAccount if missing.
 * @param {string} coaAccountId 
 * @returns {Promise<number>}
 */
const getAccountBalance = async (coaAccountId) => {
  if (coaAccountId) {
    const account = await ChartOfAccount.findById(coaAccountId);
    if (account && Number(account.openingBalance || 0) === 0) {
      const BankAccount = require('../models/BankAccount');
      const bankAccount = await BankAccount.findOne({ coaAccountId: account._id }).lean();
      if (bankAccount && Number(bankAccount.openingBalance || 0) > 0) {
 ...
 */
const getAccountLedger = async (coaAccountId, companyId, { from, to } = {}) => {
  const ledgerService = require('./ledger.service');
  const history = await ledgerService.getLedgerHistory(coaAccountId.toString(), {
    companyId,
    from,
    to
  });
 ...
 */
const createLinkedCoaAccount = async (companyId, accountName, accountType, openingBalance = 0) => {
