...
const createJournalEntry = async (req, res, next) => {
  try {
    const payload = {
      ...req.body,
      branchId: req.branchId || req.body.branchId || req.user?.branchId || null
    };
    const entry = await journalEntryService.createJournalEntry(payload, req.user._id);
...
const listJournalEntries = async (req, res, next) => {
  try {
    ...
    const entries = await journalEntryService.listJournalEntries(companyId, { financialYearId, branchId, from, to });
