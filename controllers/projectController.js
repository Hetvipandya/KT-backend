...
exports.createProject = async (req, res) => {
  try {
    ...
    let employees = req.body.employees !== undefined ? parseArrayField(req.body.employees) : undefined;
    let interns = req.body.interns !== undefined ? parseArrayField(req.body.interns) : undefined;

    // Resolve teamLeadUser / teamLeadEmployee if teamLead provided
...
exports.assignInterns = async (req, res) => {
  try {
    ...
    // Empty array is allowed because user may remove all interns
    if (!Array.isArray(internIds)) {
      return res.status(400).json({
        success: false,
        message: "internIds must be an array",
      });
    }
    ...
    console.error("assignInterns error:", error);
