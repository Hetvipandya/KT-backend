  ...
  try {
    ...
    if (attendance) {
      ...
      if (attendance.checkOutTime) {
        ...
        hasActiveBreak ||
        (currentSession && currentSession.status === "break")
      ) {
    ...
    const assignedTasks = await TaskManagement.find({
    ...
    const remainingLeave = leaveBalanceDoc
      ? leaveBalanceDoc.remainingLeaves
      : 20;

    // 4. Upcoming Birthdays (all employees)
    const allUsers = await User.find({ isActive: true }).select(
      "_id name dob role department",
    );
    const allEmployees = await Employee.find().select(
      "userId profileImage designation dob",
    );
    ...
    const holidays = await Holiday.find({
      holidayDate: { $gte: new Date(now.setHours(0, 0, 0, 0)) },
    })
      .sort({ holidayDate: 1 })
      .limit(10);
    ...
    const pendingRequests = myLeaves.filter((l) =>
      l.status.includes("pending"),
    ).length;
    const upcomingApprovedLeave = myLeaves.filter(
...
exports.getEmployeeAttendanceTimeline = async (req, res) => {
  try {
    ...
    const totalWorkingHours = records.reduce(
      (acc, curr) => acc + (curr.totalWorkTime || 0),
      0,
    );
    const averageWorkingHours =
      totalDays > 0 ? Number((totalWorkingHours / totalDays).toFixed(2)) : 0;
    ...
    const timelineData = records.map((r) => {
      ...
      if (r.checkInTime) {
        ...
        const checkoutMin = r.checkOutTime
          ? new Date(r.checkOutTime).getHours() * 60 +
            new Date(r.checkOutTime).getMinutes()
...
exports.endAttendanceSession = async (req, res) => {
  try {
    ...
    if (session) {
      session.status =
        reason === "auto_checkout" ? "auto_checkout" : "terminated";
...
exports.updateScreenshotSettings = async (req, res) => {
  try {
    ...
    if (typeof isMonitoringEnabled === "boolean")
      settings.isMonitoringEnabled = isMonitoringEnabled;
...
exports.getScreenshotsForAdmin = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, count: screenshots.length, data: screenshots });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.logInactivityAlert = async (req, res) => {
  try {
    ...
    const hrAdminUsers = await User.find({
      role: { $in: ["hr", "admin"] },
    }).select("_id");
    ...
    return res.status(201).json({
      success: true,
      message: "Inactivity event logged",
      eventId: event._id,
    });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
...
exports.getEmployeeTaskManagement = async (req, res) => {
  try {
    ...
    const sanitizedTasks = tasks.map((t) =>
      sanitizeTaskWithAttachments(t, req),
    );
...
exports.updateEmployeeTaskDetails = async (req, res) => {
  try {
    ...
    const task = await TaskManagement.findOne({
      _id: taskId,
      assignedEmployee: userId,
    });
    if (!task) {
...
exports.getEmployeeLeaveOverview = async (req, res) => {
  try {
    ...
    const leaves = await Leave.find({ employeeId: userId }).sort({
      createdAt: -1,
    });

    const casualLeaves = leaves
      .filter((l) => l.leaveType === "casual" && l.status === "approved")
      .reduce((a, b) => a + b.totalDays, 0);
